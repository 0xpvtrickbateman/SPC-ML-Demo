...
2. Give the training and scoring notebooks the same serverless environment version and dependencies in their Environment panels. The Standard base lacks statsmodels, and the scorer refuses a bundle saved with a different scikit-learn version. Add `-r /Workspace/<your folder>/requirements-demo-lock.txt` to both to reproduce the recorded results, or at minimum `statsmodels==0.14.6` to both.
3. The notebook defaults to `OUTPUT_SCHEMA = "ml_statistical_process_controls.demo_schema"`. Explicitly set `OUTPUT_SCHEMA = ""` and leave `UC_MODEL_NAME = ""` for a first analytical run without Delta writes. MLflow logs when installed unless you set `ENABLE_MLFLOW = False`. Cell 19 saves the portable bundle first; an MLflow failure is printed with `MLFLOW_STATUS` and does not stop later cells, so check that status before claiming an MLflow URI.
4. Set environment variable `SPC_DEMO_MODEL_DIR` to an accessible persistent directory before training if reuse must survive the compute session. The local default is `artifacts/saved_forecast`; on Databricks this relative path resolves under the notebook's folder for both notebooks, so verify it is writable. A permitted Unity Catalog volume can use `/Volumes/ml_statistical_process_controls/demo_schema/<volume>/saved_forecast`; replace `<volume>` with an existing authorized volume. Where environment variables cannot be set, assign `os.environ["SPC_DEMO_MODEL_DIR"]` before Cell 19 and set the scorer's `saved_model_directory` widget to the same path.
5. Run all 22 cells. Inspect the reconciliation assertions, chronological model comparisons, future dates, pending review queue, monitoring output and artifact-save result.
6. Retain the bundle's `forecast.joblib`, `manifest.json`, `sample_inputs.json` and `expected_predictions.json` together. The manifest records feature order/types, dependency versions, integrity hash and an MLflow model URI when logging actually succeeds. This loadable bundle is required for the reuse demonstration; MLflow and registry registration are optional mechanisms.
