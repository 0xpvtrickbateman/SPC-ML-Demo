"""Exercise the notebook's actual drift-row construction with nonfinite inputs."""
import contextlib
import io
...
def evidence(reference, current):
    ...
    ns = dict(np=np, pd=pd, DATASET_ID="finite-regression", forecast_df=inputs,
              forecast_train=inputs.run_date.lt(current_dates[0]),
              forecast_results_df=results, saved_forecast_manifest=None)
    with contextlib.redirect_stdout(io.StringIO()):
        exec(compile(cell, str(ROOT / "notebooks/SPC_ML_Demo.py"), "exec"), ns)
    rows = ns["drift_inputs_df"]
...
cases = [
    ...
    ("empty current", clean, np.array([]), 25, 0, np.nan, False),
...
print(f"PASS: actual drift evidence rows for {len(cases)} finite/nonfinite coverage cases and finite-only score equivalence.")
